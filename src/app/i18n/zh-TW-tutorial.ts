...
　　　　卡牌／牌堆：右鍵抽入手牌・派牌；F＝單卡翻面／牌堆整疊翻轉；骰子＝右鍵或 R
...
・鍵盤（選取後）：WASD 移動、Shift＋WASD 面向、Q／E 旋轉、R 擲骰／洗牌／重置面向、PageUp／Down 高度、F 翻面、L 鎖定、H 顯隱（GM）、T 集中到滑鼠、Delete、Ctrl＋C　Ctrl＋X　Ctrl＋V、Ctrl＋Z／Y
...
・**快捷鍵**：右鍵選單標示 Ctrl＋C／X／V（暫存 Token 為 Ctrl＋Shift＋V）；骰子／硬幣改按 **R** 重擲（**F** 仍翻單卡／整疊牌堆）`,
  'changelog.2026sepMid': `2026/09/08–10 圖庫刪除・角色卡編輯
・**圖片庫／曲庫刪除**：選取後刪除會從本房與目前連線玩家一併移除，且不會被 P2P 再傳回來；重新匯入同一檔可還原。曲庫亦可右鍵刪除單首
・**ZIP／資料夾還原**：打包的音訊／影片與卡圖一樣用雜湊檔名還原，讀檔後較不易缺檔
・**勾選欄**：未勾選時仍顯示選項文字（Token 懸停、角色卡、倉庫）；\`開|關\` 成對時未勾顯示關側
・**角色卡編輯**：編輯 MODE 更緊湊（選項／筆記／網址不再撐大卡片）；筆記欄可拉高並記住高度；「新增項目」在「切換編輯」左側
・**圖片庫**：避免誤選文字，方便框選圖片`,
